...
public class UserController implements Serializable {
    ...
    public void login() {
        ...
        User result;
        try {
            result = (User) em.createNamedQuery("User.findByName").setParameter("name", name).getSingleResult();
        } catch (Exception e) {
            msg = "不存在该用户，请检查用户名";
            return;
        }
        if (result.getPassport().equals(passport)) {
            current = result;
            msg = "欢迎您，" + current.getName();
            iflogin = true;

            if (result.getName().equals("manager")) {
                ifManager = true;
            }
        } else {
            msg = "密码错误，请检查密码";
    ...
    public String create() {
        try {
            int newid =getFacade().findMaxId();
            current = new User(newid,name,passport);
            selectedItemIndex = -1;
